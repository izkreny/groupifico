...
RSpec.describe EventPolicy, type: :policy do
  ...
  describe_rule :create? do
    ...
    let(:record)  { build(:event, group:, creator: actor) }
    let(:context) { { user: actor.user } }
    ...
    failed "for the manager of another event, whose column the row leaves blank" do
      before { create(:event, group:, manager: actor) }
    end
  end
  ...
  describe_rule :update? do
    ...
    succeed "for the event's own manager, who holds no role" do
      let(:record) { create(:event, group:, manager: actor) }
    end
    ...
    failed "for the event's creator, holding no role" do
      let(:record) { create(:event, group:, creator: actor) }
    end
  end
  ...
  describe "the relation scope" do
    it "excludes events of a group the user has left" do
      user = create(:user)
      gone = create(:event, group: create(:member, :inactive, user:).group)
      kept = create(:event, group: create(:member, :active, user:).group)

      scoped = described_class.new(nil, user:).apply_scope(Event.all, type: :active_record_relation)
