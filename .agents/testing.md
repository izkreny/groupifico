...
Every controller action is asserted in two contexts, signed in and not signed in. The signed-out surface is whatever declares `allow_unauthenticated_access` - today the sign-in form (`sessions#new`/`#create`) and the emailed link's two halves (`sign_ins#show`/`#create`). Those actions assert their signed-out rendering; every other action's signed-out context asserts the redirect to the login page. When an action's access changes, its paired contexts change with it.
