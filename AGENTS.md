...
- How a dependency moves lives in [`.agents/dependencies.md`](.agents/dependencies.md): the update order, the `BUNDLED WITH` trap, the gems upstream holds below their latest release, and what a Dependabot pull request is for. Read it before bumping a gem, editing `Gemfile` or `Gemfile.lock`, or acting on a Dependabot pull request.
