...
- **Nullability renders, on every attribute, as `NN` or `NULL`.** `NN` is not a new token: the February ERD already used it in the default block, alongside `AI`. Both states are marked rather than just one, because among the 28 columns the diagram renders 13 are `NOT NULL` and 15 are nullable, so a blank would mean "nullable" and "not filled in yet" at the same time. At a 13 to 15 split there is no safe default to leave unmarked. The cost is accepted deliberately: every row now carries a comment where many carried none, so every entity box widens by about the width of `NULL`, uniformly.
...
- Write the legend into `README.md` **outside the fenced mermaid block**, beside the diagram, replacing the single `[!IMPORTANT]` line. Outside is not a detail: inside the fence a legend is either a `%%` comment, which does not render at all, or a fake entity box, which puts documentation into the picture it documents. Outside, it is markdown, so it can be a table that defines every token: `PK`, `UK`, `FK`, `ENUM`, `NN`, `NULL`, `AI`.
