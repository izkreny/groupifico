...
- Tighten `RegistrationPolicy`: `show?` for every member; `create?` and `update?` for every member on their own registration, for `can_manage?(:events)` on anybody's, and for the event's manager on `create?` alone; `destroy?` for `can_manage?(:events)` only, since a registration is answered rather than withdrawn
...
- **Does a member refused a write get a `403`, or the redirect every other refusal gets?** Both issues' criteria say `403` and so does `docs/AUTHORIZATION.md`, but `ApplicationController#deny_access` sends every denial that is not `not_found` to `redirect_to root_path` with an alert, for the reason its comment records: a refused Turbo submission landing back on the form looks like a dead button. The paused member is refused exactly that way today. Recommendation: keep the redirect, read "403 rather than 404" as "refused plainly rather than concealed", and correct the sentence in `docs/AUTHORIZATION.md` here. A real `403` would change every refusal in the application rather than only these.
- **May a member write `reserved` or `invited` onto their own registration?** `Registration`'s statuses are `reserved`, `invited`, `yes`, `maybe` and `no`, and only the last three are an answer. The table's row is "Set or change their own attendance answer", so writing either of the first two onto one's own registration exceeds it: those are the states an administrator or a manager puts somebody into. Recommendation: permit a member only `yes`, `maybe` and `no` on their own registration, which makes the rule mirror the row rather than exceed it, and leave `reserved` and `invited` to the three roles and the manager. Enforcing it is a status check inside `RegistrationPolicy#update?` rather than a new cell.
