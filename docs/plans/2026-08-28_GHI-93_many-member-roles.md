...
- **Should the role picker stay on the member form at all on this branch?** No, and no replacement is built either. Settled by the owner on the PR: *"This is backend issue, leave all frontend and UI work for later."* The select, the role line in the member partial and the helper that feeds them are removed, since all three read the enum this branch drops; a picker that can express several roles is deferred with no issue filed for it yet, which leaves the eighth acceptance criterion on #93 half met here.
