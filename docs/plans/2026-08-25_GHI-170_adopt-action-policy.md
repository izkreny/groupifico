...
- **Should `verify_authorized_scoped` be enabled too?** Yes, but in #172 rather than here. Settled by the owner in review: "OK, put it inside the #172". The guard counts `authorized_scope` calls and raises `ActionPolicy::UnscopedAction`; enabled bare it fires on `show`, `edit`, `update` and `destroy`, which scope nothing, so it is constrained to `only: :index`. It stays off this branch because nothing here scopes anything and a guard nobody has watched fire is not a guard. Recorded as an acceptance criterion on #172.
