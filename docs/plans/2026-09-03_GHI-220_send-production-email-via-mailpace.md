...
The gem is an engine whose only initializer calls `ActionMailer::Base.add_delivery_method(:mailpace, Mailpace::DeliveryMethod)`, ordered `before: 'action_mailer.set_configs'`, so a `config.action_mailer.mailpace_settings` written in an environment file is picked up the way `smtp_settings` would be. `config/application.rb` already loads `rails/all`, so the gem's `require 'action_mailbox/engine'` pulls in nothing this app does not already have. It does add a route: railties unshifts an engine's own `config/routes.rb` into the application's route set, so `POST /rails/action_mailbox/mailpace/inbound_emails` is drawn and `bin/rails routes` lists it, alongside the ingresses Action Mailbox already ships. The unset `config.action_mailbox.ingress` governs the response rather than the routing, so the endpoint exists and answers 404 from `ActionMailbox::BaseController#ensure_configured`, which runs before the subclass's password check.
