...
- Settled in the terminal on 2026-09-14, after `bin/rspec spec/styleguide` failed its accessibility example on daisyUI's light `--color-error` as field-error text (2.87:1): change the colour here, or track it? Decided by the owner: no colour changes on this branch; #299 tracks the defect, blocked by #257's theme, and the third `## Verification` gate passes when the accessibility example is the one failure and its report names #299's violation alone.
