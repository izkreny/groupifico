...
Cuprite is the one alternative the guide documents, in the same section that documents the default: add the gem, `require "capybara/cuprite"`, `driven_by :cuprite`. Everything else in the corpus is dead (Poltergeist, Apparition) or a second runtime (Playwright, Cypress), whose gain over Cuprite that would matter here, driving Firefox and WebKit as well as Chromium, this Chromium-only suite does not use, and which adds a Node toolchain to install and keep in step on every machine that runs `bin/ci`.
