...
Cuprite is the one alternative the guide documents, in the same section that documents the default: add the gem, `require "capybara/cuprite"`, `driven_by :cuprite`. Everything else in the corpus is dead (Poltergeist, Apparition) or a second runtime (Playwright, Cypress), which buys nothing Cuprite's own CDP session does not already give and adds a Node toolchain to install and keep in step on every machine that runs `bin/ci`.
