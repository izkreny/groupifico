...
> **Correction, 2026-09-07 (#269).** The decision above stands and the configuration is unchanged, but `labels: []` does not have the effect this record describes. It is the mechanism GitHub documents for suppressing every label, and GitHub's backend ignores it: [dependabot/dependabot-core#11783](https://github.com/dependabot/dependabot-core/issues/11783) is open, a recurrence of [#6858](https://github.com/dependabot/dependabot-core/issues/6858), and the label decision is made in the closed-source job generator rather than in `dependabot-core`'s own labeler, which handles an empty list correctly. So `dependencies` and `ruby` came back and sit on every Dependabot pull request, no change to that config file fixes it, and the standing call is to leave them alone. [`.agents/dependencies.md`](../../.agents/dependencies.md) carries that rule, written to cover ecosystems this repository has not added yet.
