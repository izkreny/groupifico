...
`rails generate authentication` has one variant and it is password based. There is no magic-link support, no one-time-code support, and no OTP primitive anywhere in Rails; the only matches for those terms in the whole framework are a Permissions-Policy directive name and the `--otp` flag `gem push` takes for its own two-factor release flow.

What Rails does ship and this uses: `rate_limit(to:, within:, by:, with:, store:, name:, scope:)` in `actionpack/lib/action_controller/metal/rate_limiting.rb`, where `by:` defaults to `request.remote_ip` and returning something with a `cache_key` scopes it per account instead.
...
Reading their source turned the argument from a preference into a conclusion, because it showed what adoption would have meant inheriting. `mikker/passwordless` stored raw tokens in the database for every 0.x release and dropped the column only in 1.0. Its `claim!` checks whether a row is claimed and then calls `touch`, with no transaction, no lock and no constraint, so two concurrent requests both pass. Its sign-in link guards only `HEAD` requests, under a comment that names the scanner problem correctly and then solves the wrong half of it. Its default delivery is `deliver_now`, inside the request. Its token generator is `CHARS.sample(6, random: SecureRandom).join`, and `Array#sample` draws without replacement, so the real keyspace is roughly 2^30 rather than 36^6.

`authentication-zero` is the useful one, as a blueprint rather than a dependency. It is by the author of Rails' own authentication generator, so its `--passwordless` output is the same lineage as the code already here: a small controller over `find_signed!`, one view, one mailer view, and a migration whose table carries a single foreign key. Its working lines are `@user.sessions.create!` and `cookies.signed.permanent[...]`, which is what `start_new_session_for` already does.
