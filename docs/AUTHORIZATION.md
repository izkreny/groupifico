...
**A group comes into being two ways, and only one of them is an authorization question.** Any signed-in user may create a group and becomes its `owner` in the same request, because there is no group to belong to yet; `GroupPolicy` skips both pre-checks for `index?`, `new?` and `create?` for exactly that reason, and `GroupPolicy#create?` is the sole owner of that question. The other way is signing up: a visitor names a group and an address, and confirming the emailed link creates the user, the group and the owner membership together. That route raises no authorization question at all, because there is no actor until the act creates one, so nothing about it belongs in a capability table. Neither route is reachable from the other's situation - `refuse_authenticated` turns a signed-in visitor off the sign-up pages and `require_authentication` turns a signed-out one off `/groups/new` - and what divides them is whether a session already proves the address, never who may have a group.

**Listing groups is the other half of `create?`'s skip.** Every signed-in user may ask, and `GroupPolicy`'s scope answers with the groups they are a member of, so the tables below decide what a member may do with a group rather than which groups they can see at all.
...
| Remove a registration, their own included              |        |   x   |       x       |          x           |                       |         |
...
**A registration is answered, never withdrawn.** The statuses are `reserved`, `invited`, `yes`, `maybe` and `no`, and only the last three are an answer, so a member's own writes are limited to those three: `reserved` and `invited` are what somebody filling the event puts you into. Declining is the `no` answer rather than a deletion, which is why the `member` column holds no removal at all: `RegistrationPolicy#destroy?` is the three roles' alone, and it covers every registration, their own included.

**An answer needs an event that can still take one.** Every column may set their own attendance answer, and none of them may set one on an event somebody has concluded or called off: `Event#open_to_answers?` is the question, and `Registration` refuses the write. No cell above is narrowed by this, because it says nothing about who - a group's owner is refused it exactly as a plain member is, and for the same reason. It sits on the model rather than in `RegistrationPolicy` on the same split as the last-active-owner guard below: an authorization rule answers "may this actor", and "this event is over" is not a fact about the actor. Recording it here is the other half of that precedent, so a rule written from the answer row later is written knowing what bounds it.

**A registration is made for a member, never by one.** The table gives `member` the answer and no row of it gives anybody the registering of themselves, which is deliberate rather than an omission: a place is `reserved` for a member or the member is `invited`, and answering is what the member does with it afterwards. So `RegistrationPolicy#create?` is the invitation row alone, and `own?` appears on `update?`, where the answer is.
