class AppFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def field(attribute, as: :text_field, hint: nil, **options)
    ...
    @template.safe_join [
      ...
      control(as, attribute, options, invalid: invalid, describedby: (note_id(attribute, invalid) if note.present?)),
  ...
  private
    ...
    def control(as, attribute, options, invalid:, describedby:)
      component, error = as == :text_area ? [ "textarea", "textarea-error" ] : [ "input", "input-error" ]

      public_send as, attribute, options.merge(
