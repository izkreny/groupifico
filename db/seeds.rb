...
active_records_models = [ User, UserProfile, Group, Member, Event, Attendee, Address ]
database_records_count = active_records_models.map(&:count).sum
if database_records_count.zero?
  $stdout.puts "Loading sample data from the 'db/seeds.rb' file..."
  populate_empty_database
else
  $stdout.puts "Database is not empty! To load sample data from the 'db/seeds.rb' file, use 'rails db:reset' or 'rails db:seed:replant' command."
end
